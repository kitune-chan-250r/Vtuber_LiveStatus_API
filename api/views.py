...
class ScheduledLiveList(APIView):
    def get(self, request, format=None):
        snippets = On_Live.objects.all()
        serializer = ScheduledLive_POST_Serializer(snippets, many=True)
        return Response(serializer.data)
...
class ScheduledLiveDetail(APIView):
    ...
    def get_object(self, pk):
        try:
            return ScheduledLive.objects.get(pk=pk)
        except ScheduledLive.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = ScheduledLiveSerializer(snippet)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = ScheduledLiveSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ScheduledLiveListView(generics.ListCreateAPIView):
    queryset = ScheduledLive.objects.all()
    serializer_class = ScheduledLiveSerializer
    permission_classes = (AllowAny, )
    def get_queryset(self):
        queryset = ScheduledLive.objects.all()
        pr = self.request.query_params.get('pr', None)
